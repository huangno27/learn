...
def find_MaxPage():
    all_url = 'https://www.7373jj.com/html/news/69/'
    start_html = requests.get(all_url,headers = header)
    #找寻最大页数
    soup = BeautifulSoup(start_html.text,"html.parser")
    page = soup.find_all('a',class_='text-muted pull-right')
...
def Download(href,header,title,path):
    ...
    for num in range(1,int(pic_max)+1):
        ...
        pic_url = mess.find('class',alt = title)
        html = requests.get(pic_url['data-original'],headers = header)
...
if __name__=='__main__':
    ...
    same_url = 'https://www.7373jj.com/html/news/69/'
    ...
    for n in range(1,int(max_page)+1):
        ...
        soup = BeautifulSoup(start_html.text, "html.parser")
        all_a = soup.find('div', class_='active').find_all('a', target='_blank')
        for a in all_a:
